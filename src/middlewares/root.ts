...
export default function (req: Request, res: Response, next: NextFunction) {
  ...
  if (!(req.session as any).Cart) {
    ...
    (req.session as any).Cart = createEmptyCartData() as CartData;
  }

  res.locals.Cart = (req.session as any).Cart;
  ...
  dc.push(
    "settings",
    ContentData.appSettings({ language: culture.language }).then((settings) => {
      if (!settings) {
  ...
  function createPageMenu() {
    ...
    return ContentData.pages({ limit: 10, language: culture.language }).then(
      (pages) => {
        menu = menu.concat(
          pages &&
            pages.items &&
