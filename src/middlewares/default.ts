...
export default function (_req: Request, res: Response, next: NextFunction) {
    ...
    dc.push('latestArticles', ContentData.articles({ limit: 6, language: culture.language }));
        ...
        ContentData.shopCategories({ limit: 3, language: culture.language, isPromoted: true })
            .then(categories => {
                if (categories && categories.items && categories.items.length) {
                    const tasks: any[] = [];
                    categories.items.forEach(item => tasks.push(ContentData.shopProducts({ limit: config.countItemsPerPromotedCategory, language: culture.language, categoryId: item.id, order: '-createdAt' })));
