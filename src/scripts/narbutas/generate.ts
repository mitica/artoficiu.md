  ...
  if (explore) {
    await seriesPromise(items, async (item) => {
      ...
      if (type === PageType.PAGE) {
        ...
        $(".ad-thumb-list a").each((_index, element: cheerio.TagElement) => {
          const img = $("img", element);
          images.push({
            largeUrl: formatUrl(
              element.attribs["href"].replace(
                /_resampled\/croppedimage\d+-/i,
                ""
              )
            ),
...
async function getPageMenu(url: string, level: number): Promise<MenuLink[]> {
  ...
  page(`#mid_left .sme${level} > a`).each(
    (_index, element: cheerio.TagElement) => {
      const item: MenuLink = {
        url: HOST + element.attribs["href"],
        id: slug(
          element.attribs["href"].replace(/\//g, "").trim().toLowerCase()
        ),
