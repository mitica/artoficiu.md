...
route.get("/actions/cart/remove/:id", function (req: Request, res: Response) {
  ...
  const cart = (req.session as any).Cart as CartData;
...
route.post("/actions/cart/add", function (req: Request, res: Response) {
  ...
  const variantId =
    req.query.variantId || req.query.variantid || req.query.variant_id;
...
route.post("/actions/email_us", function (req: Request, res: Response) {
  ...
  let subject =
    req.body.subject || util.format(__("message_from_format"), contact);
  ...
  recaptchaValidate(recaptchaValue, ip)
    .then((isHuman) => {
      if (!isHuman) {
...
route.post("/actions/checkout", function (req: Request, res: Response) {
  ...
  OrdersRepository.create(order)
    .then((createdOrder) => {
      ...
      const invoicePdfLink = canonical(
        links.invoices.invoicePdf(createdOrder.id)
      );

      const emailSubject = __("invoice_email_subject");
      const emailBody = __("invoice_email_body").replace(
        "${invoice_pdf_link}",
        invoicePdfLink
      );
